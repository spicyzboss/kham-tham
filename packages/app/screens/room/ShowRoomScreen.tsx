...
export default function ShowRoomScreen() {
  ...
  const { data, error } = useSWR(token ? `http://192.168.0.100:3000/room/info/${roomId}` : null, fetchRooms);
    ...
    <YStack f={1} backgroundColor={'#000'}>
      <YStack p="$3" space="$2" ai="center" mt="$6">
        ...
        {showCode ? (
          ...
          <ContainerShowPlayer loading={loading} playTheRoom={playTheRoom} player={data.PlayerJoinedRoom} />
        )}
      </YStack>
    </YStack>
  );
